...
class GraphicsEditor(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Picker Editor - Center Origin")
        ...
        # 垂直線 (Y軸)
        vertical_line = QtWidgets.QGraphicsLineItem(0, -self.height / 2, 0, self.height / 2)
        vertical_line.setPen(guideline_pen)
        vertical_line.setZValue(-0.5) # 背景より手前、モジュールより奥
        self.scene.addItem(vertical_line)
        
        # 水平線 (X軸)
        horizontal_line = QtWidgets.QGraphicsLineItem(-self.width / 2, 0, self.width / 2, 0)
        horizontal_line.setPen(guideline_pen)
        horizontal_line.setZValue(-0.5) # 背景より手前、モジュールより奥
