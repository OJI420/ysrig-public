...
class Guide(guide_base.GuideBase):
    ...
    def create(self):
        ...
        joint_names = self.joint_names + core.create_numbered_names(self.grp_name, self.joint_count - 1, gb=False)
    ...
    def connect(self):
        if self.joint_count > 2:
            core.connect_equal_point(self.guide_joints[1:])
    ...
    def lock_attributes(self):
        ...
        self.settings_node, ["Orient", "GoalBone", "Mirror"]
        ]
    ...
    def apply_settings(self, root_matrix=[0, 0, 0, 0, 0, 0, 1], guide_positions=[[5, 0, 0], [40, 0, 0]],
    connect_type=1, translate_enabled=True):
        if self.error:
            return
        ...
        for i, axis in enumerate("XY"):
            cmds.setAttr(f"{self.guide_joints[1]}.translate{axis}", guide_positions[0][i])
            cmds.setAttr(f"{self.guide_joints[-1]}.translate{axis}", guide_positions[1][i])
...
def build(data):
    ...
    G = Guide(data["GroupName"], data["JointCount"] - 1, data["ParentName"], data["Side"], data["JointName"][:1])
    G.apply_settings(root_matrix=[*pos, *rot, scl[0]], connect_type=data["ConnectType"], translate_enabled=data["TranslateEnabled"])
